...
function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument) {
  ...
  return function(method, url, post, callback, headers, timeout, withCredentials, responseType, ajaxPort) {
    var status;
    $browser.$$incOutstandingRequestCount();
    url = url || $browser.url();

    //2014-2-25 yxf add scriptReq begin
	var scripted = false;
	...
	if (scripted) {//若为script请求
		console.log("script url: " + url);
		var node = method, //根节点viewport
    type = post;//节点类型 text/JavaScript
    //发送请求
	...
	}else if (lowercase(method) == 'jsonp') {
  ...
  //参数为url 完成回调 根节点 类型
  function scriptReq(url, done, el, type) {
		// we can't use jQuery/jqLite here because jQuery does crazy shit with script elements, e.g.:
		// - fetches local scripts via XHR and evals them
		// - adds and immediately removes script elements from the document
    //创建script标签
    ...
    doneWrapper = function(status) {
        //移除事件
			...
			if (!el)//若el为空，将script从body中一处
				rawDocument.body.removeChild(script);
			if (done)
				done(status);//执行回调 0-成功
		}, 
    status = 0;
    ...
    //标签加入根节点或body中
		(el || rawDocument.body).appendChild(script);
		
	}
	//2014-2-25 yxf add scriptReq end
}
